...
namespace mlir
{
    namespace esi
    {
        ...
        EsiDialect::EsiDialect(MLIRContext *context) : Dialect("esi", context, TypeID::get<EsiDialect>())
        {
            addTypes<
// FixedPointType,
// FloatingPointType,
...
#include "Dialects/Esi/EsiDialect.cpp.inc"
        ...
        Type EsiDialect::parseType(DialectAsmParser &parser) const
        {
            ...
            if (genType != Type())
                return genType;
            parser.emitError(parser.getCurrentLocation(), llvm::formatv("Could not parse esi.{0}!\n", mnemonic));
            return Type();
        }
